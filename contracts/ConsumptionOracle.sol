...
/**
 * @title ConsumptionOracle
 * @dev Handles verifier claim submission and consensus finalization for energy consumption.
 * Verifiers submit signed claims for hourly consumption data, and the contract
 * aggregates submissions to reach consensus via quorum.
 * 
 * Trust Model Note: Consumption data is "research-grade" from CSV uploads or simulated
 * meter APIs. It is NOT billing-grade or meter-certified. This is acceptable for
 * evaluating matching mechanics and overhead measurement.
 */
contract ConsumptionOracle is Ownable {
    ...
    /// @notice Registry contract for verifier and consumer data
    ...
    /// @notice Mapping of consumerId => hourId => verified consumption (for Matcher queries)
    mapping(bytes32 => mapping(uint256 => uint64)) private _verifiedConsumption;

    // ============ Events ============

    /// @notice Emitted when a consumption claim is submitted
    ...
    /// @notice Emitted when consumption is finalized
    event ConsumptionFinalized(
        bytes32 indexed claimKey,
    ...
    error ConsumerNotRegistered(bytes32 consumerId);
    ...
    /**
     * @notice Submit a consumption claim for verification
     ...
     * @param energyWh The energy consumption in Wh
    ...
    function submitConsumption(
        ...
        // Derive claim key
        bytes32 claimKey = getClaimKey(consumerId, hourId);
        ...
        if (!_isConsumerRegistered(consumerId)) {
            revert ConsumerNotRegistered(consumerId);
        }

        // Validate signature and get verifier address
        address verifier = _validateSignature(consumerId, hourId, energyWh, evidenceRoot, signature);
        ...
        try registry.getVerifierIndex(bucket.snapshotId, verifier) returns (uint8 verifierIndex) {
            ...
            emit ConsumptionSubmitted(claimKey, verifier, energyWh, valueHash);
            ...
            if (baselineMode && singleVerifierOverride != address(0) && verifier == singleVerifierOverride) {
                _finalizeWithValue(claimKey, consumerId, hourId, energyWh, evidenceRoot, valueSub.verifierBitmap);
            }
        } catch {
    ...
    /**
     * @notice Check if a consumer is registered
     */
    function _isConsumerRegistered(bytes32 consumerId) internal view returns (bool) {
        try registry.getConsumer(consumerId) returns (IRegistry.Consumer memory) {
            return true;
        } catch {
            return false;
        }
    ...
    /**
     * @notice Finalize consumption claim after deadline
     * @dev Anyone can call after deadline. Calculates quorum and determines winner.
     * @param consumerId The consumer identifier
     * @param hourId The hour identifier
     */
    function finalizeConsumption(bytes32 consumerId, uint256 hourId) external {
        bytes32 claimKey = getClaimKey(consumerId, hourId);
        ClaimBucket storage bucket = _claimBuckets[claimKey];
        ...
        if (!quorumReached) {
            ...
            emit ClaimDisputed(claimKey, consumerId, hourId, "No quorum reached");
            return;
        }
    ...
    function _finalizeWithValue(
        ...
        // Store verified consumption for Matcher queries
        _verifiedConsumption[consumerId][hourId] = energyWh;

        // Distribute rewards to winners (consumption verification also earns rewards)
        treasury.distributeRewards(winnerBitmap, bucket.snapshotId, energyWh);

        emit ConsumptionFinalized(claimKey, consumerId, hourId, energyWh);
    }
    ...
    /**
     * @notice Get the claim key for a consumer and hour
     * @param consumerId The consumer identifier
     * @param hourId The hour identifier
     * @return The claim key
     */
    function getClaimKey(bytes32 consumerId, uint256 hourId) public view returns (bytes32) {
        return keccak256(abi.encodePacked(
            bytes1(0x02), // ClaimType.Consumption
            address(this),
            consumerId,
            hourId
        ));
     ...
     * @dev Used by Matcher contract to verify consumption before matching
     * @param consumerId The consumer identifier
     * @param hourId The hour identifier
